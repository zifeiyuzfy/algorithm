...
def is_target_detected(frame):
    ...
    # 进行检测
    _, classes = ssd.detect_image(frame, return_classes=True)
...
def is_target_detected(frame, ssd):
    """
    判断帧中是否检测到目标
    :param frame: 输入的帧
    :param ssd: SSD模型实例
    ...
    frame = Image.fromarray(np.uint8(frame))
    # 进行检测
    _, _, classes = ssd.detect_image(frame, return_classes=True)
    return len(classes) > 0

def get_detected_target_types(frame, ssd):
    """
    获取检测到的目标的类型
    :param frame: 输入的帧
    :param ssd: SSD模型实例
...
if __name__ == "__main__":
    ...
    count = False
    # ----------------------------------------------------------------------------------------------------------#
    #   video_path          用于指定视频的路径，这里设置为 RTMP 地址
    #   output_stream_url   表示输出视频流的 RTMP 地址
    # ----------------------------------------------------------------------------------------------------------#
    video_path = 'rtmp://localhost:1935/flv-live/test'
    output_stream_url = 'rtmp://localhost:1935/flv-live/test02'
    ...
    while True:
        ...
        # 进行检测
        result_img, boxes, classes = ssd.detect_image(frame_pil, return_classes=True)
