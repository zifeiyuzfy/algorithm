...
class FRCNN(object):
    _defaults = {
        ...
        "classes_path"  : 'model_data/voc_PedestrianAndPeople.txt',
        #---------------------------------------------------------------------#
        ...
        #-------------------------------#
        "cuda"          : True,
        "use_soft_nms": False,  # 是否开启软非极大值抑制
        "soft_nms_sigma": 0.01,  # Soft - NMS 中高斯衰减函数的参数
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.bbox_util = DecodeBox(
            self.std, self.num_classes,
            use_soft_nms=self.use_soft_nms,  # 传递参数
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, crop=False, count=False, return_classes=False):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        ...
        if return_classes:
            classes = [self.class_names[int(c)] for c in top_label]
            return image, top_boxes, classes
        return image
